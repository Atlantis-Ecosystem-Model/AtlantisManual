...
| **Parameter** | **Description** |
|---|---|
| [UseConstPrice]{style="color: orange;"} | Parameter determining whether to use constant fish prices given in `XXXsaleprice` parameter (if set to 1) or if prices should be updated dynamically using market based calculations (if set to 0). |
...
| [MARKET_intercept1, MARKET_intercept2]{style="color: orange;"} | Market-specific scalar for each species fish price (unitless). |
| [MARKET_trend1, MARKET_trend2]{style="color: orange;"} | Long term price trend coefficient constant for each market and species, to be multiplied by the simulation year number to get \$/kg. |
| [JanMarketCoefft1, FebMarketCoefft1, ...]{style="color: orange;"} | Monthly market constant (\$/kg) in fish price. |
| [MARKET_autocoefft1, MARKET_autocoefft2]{style="color: orange;"} | Market specific constants to be applied to externally read price residuals. |
...
| [YYY_flagDV]{style="color: orange;"} | Fishery specific flag determining whether to use no deemed value (=0), simple deemed value (=1), or complex deemed value (=2). See chapter 17.3.2. |
...
| [OwnQuotaXXX_sub1, OwnQuotaXXX_sub2, ...]{style="color: orange;"} | Total allowable catch of quota (in tonnes) for each subfleet of each fishery (superseding the values for TACs set in *harvest.prm*). If set to 10^12^ then no quota for that species is applied. |
...
### ***Monthly updating of boat numbers***
...
| **Parameter** | **Description** |
|---|---|
| [YYY_nlicences]{style="color: orange;"} | Numbers of licenses in each fishery, i.e. number of vessels/boats a fishery is allowed to have across all of its subfleets. |
| [YYY_buybackdate]{style="color: orange;"} | A day of the simulation run when a proportion of boats given in the `YYY_propbuyback` parameter is taken out of the fishery (bought back). If buy back is not wanted set this date to a large value. |
| [flagsupp_allowed]{style="color: orange;"} | If set to 1, subfleets are allowed to move across fisheries to simulate gear switching; if set to 2 subfleets are considered to have flexible gear (see chapter 17.4.1 for further details). |
...
## **Scheduling of fishing effort and final effort allocation**
...
![[]{#_Toc498434139 .anchor}**Figure 12.** The monthly effort scalar calculation. See explanation in the text above.](media/image14.png){width="5.004166666666666in" height="3.9270833333333335in"}
...
2)  Next, each box is weighted by their *mpasc~J,j~* ([MPAYYY]{style="color: orange;"} parameter from the *harvest.prm* file, see chapter 16.4), which will change the actual CPUE in this box if some of the box area is closed to fishing. This scaled value is used to calculate the ideal CPUE based effort (*CPUE~eff~*) given the latest CPUE knowledge, monthly effort plan (as explained above) and fisher behavioural flexibility.
...
1. Update fisher response to latest price and CPUE information when making the annual fishing plan (see chapter 17.5.1).
...
| **Parameter** | **Description** |
|---|---|
| [MultiPlanEffort]{style="color: orange;"} | If set to 1, then baseline economic Atlantis model is used to determine effort allocation. If set to 0, the Dan Holland economics model is used (see chapter 17.5.3 for further details). |
| [JanYYYEffort_sub1, FebYYYEffort_sub1, ...]{style="color: orange;"} | Historical average fishing effort (in effort days per box per month) -- given for each subfleet of each fishery. This represents a part of the subfleet's "black books". |
| [JanCatchXXX_sub1, FebCatchXXX_sub1, ...]{style="color: orange;"} | Historical average catch per species (kg of catch per box per month) -- given for each subfleet of each fishery. This represents a part of the subfleet's "black books". |
| [JanYYYCPUE_sub1, FebYYYCPUE_sub1, ...]{style="color: orange;"} | Historical average CPUE (kg of catch per one day of effort per box per month) -- given for each subfleet of each fishery. This represents a part of the subfleet's "black books". |
| [OrigEconCalc]{style="color: orange;"} | If set to 1 then the expected yearly profit is calculated as earnings minus costs. If set to 0, then profit calculation takes into account CPUE data and proportional contribution of each fish in the catch (see chapter 17.5.1). |
...
| [YYY_flexweight]{style="color: orange;"} | Parameter setting behavioural flexibility of a subfleet (scalar from 0 to 1). See callout in chapter 17.5.3 for an explanation of how this parameter is used in three different calculations. |
