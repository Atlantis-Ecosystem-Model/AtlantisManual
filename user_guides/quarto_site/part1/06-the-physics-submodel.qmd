...
Most of the details in the Atlantis Physics submodel are based on the Port Phillip Bay Integrated Model (PPBIM) described in Murray and Parslow (1997) and Walker (1997) and on the [ERSEM](http://www.meece.eu/library/ersem.html) model described in Ebenhöh et al. (1995) (see important references @sec-comp-es-models). Atlantis users interested in understanding the physics processes deeper can learn more by consulting these references.
...
::: {#tbl-initial-conditions-tracers}

**Further description of tracers and physics variables is given in the following chapters on sediment and biological processes.**
...
Examples of currently used obligatory and optional physics and biogeochemical tracers and variables and their characteristics as defined in the *initial_conditions.nc* file (collected from different Atlantis applications and used for illustration purposes only).
...
If [scale_transport]{style="color: orange;"} is set to 1, the transport will be scaled by [prcnt_exchange]{style="color: orange;"} parameter. For example, setting [prcnt_exchange]{style="color: orange;"} to 0.5 will half all the horizontal and vertical transport, in some older models this was needed to help correct for hyperdifussion. 
...
| **Forcing species distributions**<br><br>[use_move_entries]{style="color: orange;"} 1 | To change the distribution of species (for movement) you need to set the flag and then provide the following information for each group you wish to force (this example has two groups, FXX and FYY, and 2 stages)<br><br>[nforceMoveGroups]{style="color: orange;"} 4<br><br>[MoveGroupCodes]{style="color: orange;"} 4<br><br>FXX_stage_0<br><br>FXX_stage_1<br><br>FYY_stage_0<br><br>FYY_stage_1<br><br>Also list the distribution nc files - 1 file per "MoveGroupCode"<br><br>[FXX_stage_0_File.name]{style="color: orange;"} DistribFilename1.nc<br><br>[FXX_stage_1_File.name]{style="color: orange;"} DistribFilename2.nc<br><br>[FYY_stage_0_File.name]{style="color: orange;"} DistribFilename3.nc<br><br>[FYY_stage_1_File.name]{style="color: orange;"} DistribFilename4.nc<br><br>You must also provide the starting time for using the forcing (so it does not have to start on day 0 of the model simulation)<br><br>[FXX_stage_0_tstart]{style="color: orange;"} 0<br><br>[FXX_stage_1_tstart]{style="color: orange;"} 0<br><br>[FYY_stage_0_tstart]{style="color: orange;"} 0<br><br>[FYY_stage_1_tstart]{style="color: orange;"} 0<br><br>The format of the nc files and other relevant information is available in the section *Forcing the model distributions with species distribution models*. |
...
| **Get ice related information from input forcing files**<br><br>*Get_Ice()* in **aticeIO.c** | Reads in ice properties for the current time step from TS or NC forcing files<br><br>Only executed if the *initial_conditions.nc* file has the global attribute [icenz]{style="color: orange;"} (see below and [details](https://confluence.csiro.au/display/Atlantis/Ice+in+Atlantis) on wiki), which activates the ice model. |
...
The optional ice model is activated when the *initial_conditions.nc* file has the global [icenz]{style="color: orange;"} variable included. This sets up the maximum number of ice layers. The ice model requires [ice forcing files](https://confluence.csiro.au/display/Atlantis/Changed+to+prm+files) and their paths need to be identified in the *force.prm* ([nInceFiles]{style="color: orange;"} parameter in *force.prm*)
