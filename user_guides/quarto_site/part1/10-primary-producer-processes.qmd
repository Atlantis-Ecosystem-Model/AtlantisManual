...
In models that track P and C ([TrackAtomicRatio]{style="color: orange;"} =1 in *run.prm* file) primary production will also be limited by P and C. Further details are available on the [wiki](https://confluence.csiro.au/display/Atlantis/2013/04/23/New+Phosphorus+tracking).
...
The amount of light falling on the surface of the top layer is either provided through external time series or calculated in Atlantis depending on the [lim_sun_hours]{style="color: orange;"} setting (described in chapter 5, Physics).
...
where *mS_MA* is the algal mortality rate (day^-1^) due to abrasion by bottom stress or waves (*STRESS*), provided through external forcing or calculated in the model (see Chapter 5, Physics) and [mS_SG]{style="color: orange;"} is the seagrass mortality rate (day^-1^) due to fouling by epiphytes in water of high nutrient concentration, and *DIN* is the concentration of NH3 and NO3.
...
It is possible to include the encystment of primary producers, triggered by time of the year or temperature. This option is only modelled for groups that have [isOverWinter]{style="color: orange;"} set to 1 in the *functional_groups.csv* file and is described in further detail on the wiki [here](https://confluence.csiro.au/pages/viewpage.action?pageId=43155510).

## **9.5. Effect of temperature and salinity on primary production**

Both light saturation ([KI_XXX_T15]{style="color: orange;"}) and maximum growth rate ([mum_XXX_T15]{style="color: orange;"}) are dependent on **temperature** and will be scaled by *Tcorr* calculated based on the current water temperature as described in chapter 13.
...
| [KI_XXX_T15]{style="color: orange;"} | Light saturation of XXX at 15C (W m^-2^) - this parameter is temperature dependent (and potentially salinity dependent), and a scalar is applied based on current temperature (and salinity) conditions. |
| [mum_XXX_T15]{style="color: orange;"} | Maximum primary producer XXX growth rate at 15C (mgN mgN^-1^ day^-1^) -- this parameter is temperature dependent (and potentially salinity dependent), and a scalar is applied based on current temperature (and salinity) conditions. |
...
| [flaglight]{style="color: orange;"} | Flag indication whether light adaptation in primary producers is activated (0=no, 1=yes), It set to 1 (i.e. activated) it will increase production at low light levels, but it will also require four additional parameters: [KIOP_min]{style="color: orange;"}, [KIOP_shift]{style="color: orange;"}, [KI_avail]{style="color: orange;"}, [K_addepth]{style="color: orange;"} (see SETas model example of *biology.prm* file) |
...
| [flagnut]{style="color: orange;"} | Nutrient limitation form used if primary production is limited by several nutrients (0=Leibig's law where limitation is set by the most limiting nutrient; 1= multiplicative; 2=WQI) |
| [flagmicronut]{style="color: orange;"} | Flag indications whether there is micronutrient limitation (0=no, 1=yes)<br><br>If set to 1, half-saturation constant for the micronutrient (typically Fe) is required ([KF_XXX]{style="color: orange;"}) |
