...
| flag_macro_model | Flag indicated whetehr a more detailed seagrass model is activated, see details [here](https://atlantis-ecosystem-model.github.io/atlantis-wiki/posts/new-seagrass-modelling-option.html) (0=no, 1=yes). If the flag is set to 1 then three separate pools will be initiated and tracked for each SEAGRASS group type: main biomass, epiphyte biomass and below ground biomass |
...
::: {.callout-note}
**If [TrackAtomicRatio]{style="color: orange;"} is set to 1** in *run.prm* file and the model tracks N:P:C many additional parameters are required to describe P, and C effect on primary production. See details [here](https://atlantis-ecosystem-model.github.io/atlantis-wiki/posts/new-phosphorus-tracking.html) and sample SETas model *biology.prm* file
:::
