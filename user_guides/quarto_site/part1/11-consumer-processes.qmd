...
![](media/image33.png){width="5.216858048993876in" height="3.5660378390201224in"}
...
| [mum_XXX_T15]{style="color: orange;"} | Maximum ingestion rate of biomass pools at 15C |
| [C_XXX_T15]{style="color: orange;"} | Clearance rate of biomass pools at 15C |
...
| [pPREY1XXX1]{style="color: orange;"}<br>[pPREY1XXX2]{style="color: orange;"}<br>[pPREY2XXX1]{style="color: orange;"}<br>[pPREY2XXX1]{style="color: orange;"} | Maximum availability of prey to a predator XXX (given as a vector with the column the prey group in the same order as in the *functional_groups.csv* file). Four values are given for each predator-prey combination: juv-juv, juv-ad, ad-juv, ad-ad. |
...
| [p_YYXXX]{style="color: orange;"} | A vector of maximum prey availability for each combination of prey YY and predator XXX identified in p_split_YY and age_structured_prey_XXX parameters |
:::
...
Further description of the rugosity related habitat cover is available on the wiki dealing with "Calculation of rugosity related habitat refuge" - [here](https://atlantis-ecosystem-model.github.io/atlantis-wiki/posts/calculation-of-rugosity-related-habitat-refuge.html) (it cover's models by Bozec and Blackwood in detail).
...
**Figure 11.** Habitat refuge scalar on prey availability as a function of cell area covered with suitable habitats (see box above for details on calculations of suitable cover area).](media/image34.png){width="4.425694444444445in" height="4.250694444444444in"}
...
*Effect of Bcov:*
[**Light blue**]{style="color: lighblue;"}: Acov=1, Bcov=0.4, Kcov=3
...
For **biomass pools** (CP) the *δ~habitat\ ~*refuge is available only for prey living in the **sediments** and is calculated by the depth the predators can dig into (set in KDEP_XXX) and depth of the oxygenated layer [sedoxdepth]{style="color: orange;"}, calculated and reported in the *output.nc* file, see Chapter 5.6). If [sedoxdepth]{style="color: orange;"} \< [KDEP]{style="color: orange;"} then no refuge is provided by the sediment (*δ~habitat~* = 1), if [sedoxdepth]{style="color: orange;"} \> [KDEP]{style="color: orange;"} then
...
**Figure 12.** Prey availability scalar Size_Scalar~prey~ of different sized prey for different [Kmax_coefft]{style="color: orange;"} values for humped selectivity curve.](media/image35.png){width="4.735416666666667in" height="3.763888888888889in"}
...
As Fig. 12 shows, the humped window calculation is very sensitive to the [Kmax_coefft_XXX]{style="color: orange;"} parameter, where availability of prey at lower size limit can increase very rapidly. An alternative formulation is now available as a logistic shaped curve to calculate δ~size~. This option is applied if [UseHardFeedingWindow]{style="color: orange;"} = 0 and [UseBiLogisticFeedingWindow]{style="color: orange;"}= 1 **and it is based on total length rather than SN of predator and prey**
...
**Figure 13.** Prey availability scalar δ~size~ of different sized prey for bilogistic selectivity curve.
...
**2. Depth based limitation** ([O2case]{style="color: orange;"}=1) only uses sediment depth of half oxygen mortality mD ([mD_XXX]{style="color: orange;"}, m) parameter and depth of the oxygenated sediment layer [sedoxdepth]{style="color: orange;"} (see chapter 5.6) where
...
1\) A biomass pool group XXX is infauna or a filter feeder or mobile epibenthic group (SED_EP_FF and MOB_EP_OTHER [GroupType]{style="color: orange;"}) **and** is habitat dependent ([habdependXXX]{style="color: orange;"}=1) **and** space limitation is active ([flagXXXlim]{style="color: orange;"}=1 or 2). If these three conditions are satisfied then the Crowding Effect scalar is applied to the clearance rate *C* of XXX (see chapter 10.5.3).

2\) A biomass pool group is habitat dependent (with at least some habitat preference parameters not set to 1), benthic and [flag_benthos_sediment_link]{style="color: orange;"}=1. See chapter 10.5.3 for further details.
...
| 4 | Minimum-maximum<br><br>Adopted from ERSEM, where it was used to describe fish feeding. However, it is not used in ERSEM anymore, because higher trophic level predators are not currently included in ERSEM | $$Gr_{prey} = \frac{B \cdot C \cdot mum \cdot \frac{B_{prey}^{*2}}{L + B_{prey}^{*}}}{U + \sum_{i}^{}\frac{B_{prey,i}^{*2}}{L + B_{prey}^{*}}}$$<br><br>*L* = lower prey biomass threshold for feeding by predator XX ([KL_XX]Note, that only SN and not total nitrogen values are used for search volume and handling time calculations. )<br><br>*U* = half saturation coefficient for feeding by predator XX([KU_XX]Note, that only SN and not total nitrogen values are used for search volume and handling time calculations. ) |
...
| [KST_fish]{style="color: orange;"}, [KST_shark]{style="color: orange;"}, [KST_bird]{style="color: orange;"}, [KST_mammal]{style="color: orange;"} | Scalar of respiration when relative reserve drops below Kthresh2 for the four group types FISH (and FISH_INVERT), SHARK, BIRD and MAMMAL |
:::
...
![](media/image54.png){width="4.338888888888889in" height="3.1034722222222224in"}
...
| [pR_XXX]{style="color: orange;"} | Parameter controlling preference of energy allocation to RN and SN (Fig. 15) |
...
| [flagXXXlim]{style="color: orange;"} | Flag defining what kind of space limitation should be applied to biomass pool consumers or benthic primary producers (see NOTE! in chapter 10.5.3) |
...
| [XXX_low]{style="color: orange;"}<br>[XXX_sat]{style="color: orange;"}<br>[XXXthresh]{style="color: orange;"} | Lower crowing threshold, crowding half saturation and crowding threshold (all in mgN m^-2^) used to calculate crowding effect in ERSEM based space limitation ([flagXXXlim]{style="color: orange;"}=2) |
...
| [flagdegrade]{style="color: orange;"} | Flag initiating geological habitat (REEF, FLAT, SOFT) degradation. If set to 1, additional parameters will be required as explained in chapter 10.5.3 |
| [Supp_XXX]{style="color: orange;"} | Proportion of a groups diet coming from outside the model domain (defined as a vector with one entry per box). On needed if [isSupplemented]{style="color: orange;"} is set to 1 in the *runs.prm* file. |
:::
...
| [XXX_CCC_TimeToLD50]{style="color: orange;"} | The number of seconds that it takes to kill of 50% of the population |
...
where *mL* is an stage-specific (juvenile and adult) linear mortality for CX ([mL_XXX]{style="color: orange;"}, day^-1^), *mQ* is an stage-specific quadratic mortality for CX ([mQ_XXX]{style="color: orange;"}, day^-1^), *mSt* is the starvation mortality (day^-1^, see below), *mA* is an optional acidification mortality (see below), *mS* is the optional extra mortality due to seabirds and fish not included in the model (adopted from ERSEM I, Bryant *et al*. 1995) ([mS_XXX]{style="color: orange;"}, day^-1^), *Num* is the numbers in an age group and *mortsc* is the optional external mortality scalar, provided through forcing files. See chapter on the forcing file for details on extra mortality scaling.
...
**Figure 16.** Starvation mortality (per day) for different [Kthreshm]{style="color: orange;"} and [mStarve]{style="color: orange;"} values.
...
Only one linear (*mL*) and quadratic (*mQ*) mortality parameter is provided for biomass pool groups, whereas for age-structured biomass pools separate parameters are needed for each pool. The oxygen mortality depends on the oxygen limitation scalar and the oxygen mortality rate ([mO_XXX]{style="color: orange;"}; day^-1^). The oxygen limitation mortality is determined by the *δ~O2~* oxygen scalar, described in chapter 10.2.8.
...
Linear and quadratic mortality parameters can be temperature dependent, as described in chapter 13. If a model includes acidification effects ([flagmodelpH]{style="color: orange;"}=1) then parameters imposing additional acidification induced mortality can be specified.
...
| [flagsenesce]{style="color: orange;"} | If set to 1, all individuals or biomass in age-structured groups and age-structured biomass pools that are above the maximum age die (see NOTE! in chapter 10.6.1) |
...
| [Kthreshm]{style="color: orange;"} | Threshold starvation level, when starvation mortality starts (chapter 10.6.2) |
| [turbid_refuge_XXX]{style="color: orange;"} | Flag indicating whether a group has its mortality effected by turbidity |
...
Flow of nitrogen from food intake to detritus and the ammonia pool can be illustrated with the flowchart in Fig. 17. It shows that first, the assimilation efficiency parameters determine how much of the grazed food is assimilated. The rest is sent to waste products. The waste products are then split between detritus and the ammonia fraction, where the amount of detritus is determined by the detritus proportion parameters ([FDG]{style="color: orange;"}, [FDGDL]{style="color: orange;"}, [FDGDR]{style="color: orange;"}).
...
| **Parameter** | **Description** |
|:--------------|:----------------|
| [FSPB_XXX]{style="color: orange;"} | A fraction of each age class in an age-structured groups and age-structured biomass pools that is sexually mature |
...
| [XXX_age_mat]{style="color: orange;"} | The first age group where adult behaviour is expressed in distribution, feeding, prey availability. It is modelled as hard transition and can be different from sexual maturation defined by [FSPB_XXX]{style="color: orange;"} parameters |
...
Users can supply external recruitment forcing scalar through time series files, listed in the *force.prm* file (see chapter 8). The external scalar will act as a multiplier on all other scalars calculated above. To indicate that external environmental recruitment forcing will be used set [flagtsforcerecruit]{style="color: orange;"}=1 in the *biology.prm* file. If this flag is set to 1, but no external forcing is provided then Atlantis will quit. This forcing represents environmental influence on recruitment (such as El Nino etc) and is called from *Get_Enviro_Recruit_Forcing()*.
